...
$parentModule = 'ContextCache'
if (-not $ModuleName) { $ModuleName = 'ContextCache.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ContextCache'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ContextCache.psd1"
$prereqHash = @{
